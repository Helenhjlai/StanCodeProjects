...
def get_average(pixels):
    ...
    #     g += pixel.green
    #     b += pixel.blue
    # avg_rgb.append(int(r / len(pixels)))
    # avg_rgb.append(int(g / len(pixels)))
    # avg_rgb.append(int(b / len(pixels)))
    # return avg_rgb
...
def get_best_pixel(pixels):
    ...
    """
    avg_rgb = get_average(pixels)  # a list storing average RGB value of compared pixels
    cd_list = []
    # cd_dict = {}
    for pixel in pixels:
        color_distance = get_pixel_dist(pixel, avg_rgb[0], avg_rgb[1], avg_rgb[2])
        cd_list.append((color_distance, pixel))
        # cd_dict[color_distance] = pixel
    # best_p = cd_dict[min(cd_dict)]
    # best_p = pixels[cd_list.index(min(cd_list))]
    return min(cd_list, key=lambda t: t[0])[1]
